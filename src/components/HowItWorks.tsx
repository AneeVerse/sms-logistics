...
const steps: Step[] = [
  {
    num: "01",
    title: "Request Your Solution",
    desc: "Contact us with your requirements—call, email, or submit a form.",
    src: "/images/how-it-works/1.png",
  },
  {
    num: "02",
    title: "Personalized Consultation",
    desc: "Our team reviews your needs and proposes a tailored logistics plan.",
  ...
  },
  {
    ...
    desc: "Continuous updates and prompt delivery, every time. You focus on your business; we deliver your promise.",
...
export default function HowItWorks() {
  return (
    <section
      id="how-it-works"
      className="relative py-12 sm:py-16 md:py-20 lg:py-24 bg-cover bg-center bg-no-repeat overflow-x-hidden"
      style={{
    ...
    >
      ...
      <div className="w-full px-4 sm:px-6 md:px-8 lg:px-12">
        ...
        <div className="relative mt-10 sm:mt-12 md:mt-16">
          {/* Desktop Grid View */}
          <div className="hidden lg:grid grid-cols-4 gap-x-8 xl:gap-x-10">
            {steps.map((s, i) => (
              <div key={s.num} className="relative flex flex-col items-center text-center group">
                ...
                {i < steps.length - 1 && (
                  <svg
                    viewBox="0 0 160 50"
                    className="absolute top-10 md:top-12 right-[-60px] xl:right-[-70px] w-32 xl:w-40 h-10 xl:h-12 text-white/95"
                    aria-hidden="true"
                  >
                ...
                {steps.map((s) => (
                  <div key={s.num} className="flex-shrink-0 w-72 sm:w-80">
                    <div className="flex flex-col items-center text-center">
                      {/* Circle with icon and number badge */}
                      <div className="relative h-24 w-24 sm:h-28 sm:w-28 rounded-full bg-white shadow-2xl flex items-center justify-center">
                        <Image 
                          src={s.src} 
                          alt={s.title} 
                          width={60} 
                          height={60} 
                          className="h-10 w-10 sm:h-12 sm:w-12 object-contain flip-icon" 
                        />
                        <span className="absolute top-2 right-0 h-6 w-6 sm:h-7 sm:w-7 rounded-full bg-[#28C2D5] text-white text-xs font-bold inline-flex items-center justify-center">
                          {s.num}
                        </span>
                      </div>

                      <h3 className="mt-4 sm:mt-5 text-white text-lg sm:text-xl font-semibold">{s.title}</h3>
                      <p className="mt-2 text-white/80 text-sm sm:text-base leading-relaxed px-2">{s.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
